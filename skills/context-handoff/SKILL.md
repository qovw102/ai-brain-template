...
## 與 progress.md / TODO 的分工

- `progress.md` / TODO 是 repo 的長期、可提交紀錄；任何檔案、設定、Skill、reference、script 或專案狀態變更，都仍需依 `auto-logging` 流程更新並 commit。
- 本 Skill 是對話層級的交接摘要；用途是讓下一個 thread、下一位 agent 或沒有內建 compact 的工具快速接手。
- 兩者不互相取代：`progress.md` 記錄「專案事實與歷史」，交接摘要整理「下一段對話接手所需的脈絡」。
- 若任務已造成 repo 變更，先更新 `progress.md` / TODO，再產生 handoff 摘要，避免摘要引用尚未落地的狀態。
- 若只是單純整理長對話、沒有修改 repo 或專案狀態，可以只產生 handoff 摘要，不需要為此建立空 commit。
...
3. 若本次工作已改變 repo 狀態，確認 `progress.md` / TODO 已由對應流程更新；handoff 只摘要結果，不取代專案紀錄。
4. 擷取對後續工作有用的資訊：
   - 使用者目標與最新要求；
   - 已完成事項；
   - 尚未完成事項；
...
5. 壓縮時保留事實與可執行下一步，刪除：
   ...
   - 與下一步無關的工具輸出；
   - 可由檔案或 Git 重新查到的大量內容。
6. 若摘要要給另一個 agent，最後加上「下一個 agent 應如何接手」。
7. 若摘要要寫入檔案，優先寫入使用者指定路徑；若沒有指定，不主動建立檔案，只在回覆中輸出摘要。
